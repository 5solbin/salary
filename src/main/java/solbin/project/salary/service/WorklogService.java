...
public class                                                                                                     WorklogService {
    ...
    @Transactional
    public AddWorklogResDto addWorklog(Long userId, AddWorklogReqDto dto) {
        // 유저 존재 확인
        ...
        JobType jobType = jobTypeRepository.findById(dto.getJobTypeId()).orElseThrow(
                () -> new CustomApiException("존재 하지 않는 정보 입니다.")
        );
        ...
        // 근무 기록 생성 및 dto 반환
        Worklog workLog = dto.ToEntity(user,jobType);

        user.addWorkLog(workLog);
        jobType.addWorklog(workLog);
        worklogRepository.save(workLog);

        return new AddWorklogResDto(workLog, jobType);
    }
    ...
    public GetMonthlyResDto getMonthlyInfo(Long userId, GetMonthlyReqDto reqDto) {
        ...
        for (Worklog worklog : worklogs) {
            if (checkMonth(worklog, reqDto)) {
                ...
                salary += workTime * worklog.getPayRate();
                int payRate = worklog.getPayRate();


                dailyMap.computeIfAbsent(date, k -> new ArrayList<>())
                        .add(new DailyInfo(date, startTime, endTime, payRate));
            }
        }

        String date = reqDto.getYear() + "/" + reqDto.getMonth();

        return new GetMonthlyResDto(date, dailyMap, totalWorkTime, salary);
    }

    // 객체의 날짜와 요청 받은 날짜가 일치하는지 확인하는 메서드
    public boolean checkMonth(Worklog workLog, GetMonthlyReqDto reqDto) {
    ...
    // 날짜가 일치 하는지 확인하는 메서드
    public boolean checkDate(Worklog worklog, AddWorklogReqDto dto) {
